...
function SavedContent() {
    ...
    useEffect(() => {
        const fetchLikedContent = async () => {
            try {
                const resVideo = await api.get(API_ENDPOINTS.CONTENT.GET_SAVED_VIDEO);
                setSavedVideo(resVideo.data.savedVideos);

                const resShort = await api.get(API_ENDPOINTS.CONTENT.GET_SAVED_SHORT);
                setSavedShort(resShort.data.savedShorts);
            } catch (error) {
                console.log(error);
    ...
    return (
        <div className="px-4 sm:px-6 py-6 min-h-screen mt-14 lg:mt-6">
            ...
            {/* Empty State */}
            {!savedShort?.length && !savedVideo?.length && (
